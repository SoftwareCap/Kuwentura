...
func _setup_initial_ui() -> void:
	_zone_active   = false; _board_unlocked = false; _board_opened  = false
	_timer_started = false; _puzzle_solved  = false; _reward_active = false
	_zone_failed   = false; _strikes        = 0;     _ledger_hint_shown = false

	if is_instance_valid(board_layer):       board_layer.visible      = true
	...
	if is_instance_valid(notification_ui):    notification_ui.visible    = true
	if is_instance_valid(notification_panel):
		notification_panel.visible       = false
		notification_panel.mouse_filter  = Control.MOUSE_FILTER_IGNORE
	if is_instance_valid(notification_label): notification_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	...
	if is_instance_valid(revealed_plant):     revealed_plant.visible     = false
	if is_instance_valid(revealed_pineapple): revealed_pineapple.visible = false
	...
	if is_instance_valid(detective_overlays): detective_overlays.mouse_filter = Control.MOUSE_FILTER_IGNORE
	if is_instance_valid(sidekick_overlays):  sidekick_overlays.mouse_filter  = Control.MOUSE_FILTER_IGNORE
	...
	if is_instance_valid(pause_canvas_layer):  pause_canvas_layer.process_mode  = Node.PROCESS_MODE_ALWAYS
...
func _populate_heights() -> void:
	if is_instance_valid(detective_height_label): detective_height_label.text = str(spirit_height_cm) + " cm"
	if is_instance_valid(sidekick_height_label):  sidekick_height_label.text  = str(plant_height_dali) + " Dali"
	if is_instance_valid(board_height_label):      board_height_label.text     = str(plant_height_dali) + " Dali"
...
func _set_dialogue_input_lock(locked: bool) -> void:
	...
	if is_instance_valid(x_input):
		if locked:
			...
			x_input.editable = is_sidekick and _board_opened and _board_unlocked and not _puzzle_solved and not _zone_failed
			x_input.modulate = normal_color

	if is_instance_valid(submit_button):
		submit_button.disabled = locked or not (is_sidekick and _board_opened and _board_unlocked and not _puzzle_solved and not _zone_failed)
...
func _get_backyard_intro_dialogue() -> Array[Dictionary]:
	...
	for entry in DialogueLibrary.BACKYARD_PATH_ENTER:
		lines.append(entry)
	lines.append({"speaker": "sidekick",  "text": "But the numbers here say " + str(plant_height_dali) + " Dali."})
	lines.append({"speaker": "detective", "text": "And Pina's height says " + str(spirit_height_cm) + " centimeters."})
	lines.append({"speaker": "sidekick",  "text": "How can we compare those if they're not in the same unit?"})
	return lines
...
func rpc_unlock_board_phase() -> void:
	...
	if is_instance_valid(feedback_label):
		feedback_label.text = "Enter the plant height in centimeters." if GameState.local_role == GameState.Role.SIDEKICK else "Tap the Deduction Board to view the conversion."

	if GameState.local_role == GameState.Role.SIDEKICK and is_instance_valid(board_tap_button):
	...
	_set_dialogue_input_lock(true)
	DialogueSystem.play("backyard_ledger_hint", DialogueLibrary.BACKYARD_PATH_LEDGER_HINT)
	await DialogueSystem.wait_finished("backyard_ledger_hint")
	_set_dialogue_input_lock(false)
...
func _open_board_local() -> void:
	...
	if is_instance_valid(feedback_label):
		feedback_label.text = "Convert " + str(plant_height_dali) + " Dali into centimeters." if is_sidekick else "The sidekick is solving the puzzle..."
...
func _on_submit_pressed() -> void:
	if _dialogue_input_locked or GameState.local_role != GameState.Role.SIDEKICK:
		return
	if not _board_unlocked or _zone_failed or _puzzle_solved:
		return
	var answer_text := x_input.text.strip_edges()
	if answer_text.is_empty():
		if is_instance_valid(feedback_label): feedback_label.text = "Enter an answer first."
		return
	if not answer_text.is_valid_int():
		if is_instance_valid(feedback_label): feedback_label.text = "Numbers only."
...
func _server_validate_answer(value: int) -> void:
	...
	if value == solution_cm:
		...
		var msg: String
		match _strikes:
			1: msg = "The forest grows uneasy..."
			2: msg = "The forest is watching you..."
			_: msg = "The forest consumes the path."
		rpc_apply_strike.rpc(_strikes, msg)
...
func rpc_apply_strike(strike_count: int, strike_message: String) -> void:
	...
	if is_instance_valid(feedback_label): feedback_label.text = strike_message
	if is_instance_valid(fog_overlay):
		match strike_count:
			1: fog_overlay.modulate.a = 0.18
			2: fog_overlay.modulate.a = 0.38
			_: fog_overlay.modulate.a = 0.65
	show_notification(strike_message, 2.0)


func _server_fail_zone(message: String) -> void:
	if _zone_failed: return
	_zone_failed = true
...
func rpc_puzzle_solved() -> void:
	...
	for node in [pina_spirit, detective_height_label, pineapple_plant, pineapple_fruit, sidekick_height_label]:
	...
	_set_dialogue_input_lock(true)
	DialogueSystem.play("backyard_solved", DialogueLibrary.BACKYARD_PATH_SOLVED)
	await DialogueSystem.wait_finished("backyard_solved")
	_set_dialogue_input_lock(false)
