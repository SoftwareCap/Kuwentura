...
func get_puzzle_for_zone(zone_id: String, seed: int) -> Dictionary:
	if not PUZZLE_TEMPLATES.has(zone_id):
		return {}
	
	var template = PUZZLE_TEMPLATES[zone_id]
	var generator = Callable(self, template.generate)
	return generator.call(zone_id, seed)

func _generate_algebra_puzzle(zone_id: String, seed: int) -> Dictionary:
...
func _generate_conversion_puzzle(zone_id: String, seed: int) -> Dictionary:
	var rng = RandomNumberGenerator.new()
	rng.seed = seed
	...
	return {
		...
		"host_view": {
			"sees_spirit_height": true,
			"spirit_height": spirit_height
		},
		"sidekick_view": {
			"sees_plant_dali": true,
			"plant_dali": dali_value,
			"formula": "1 Dali = 2 cm"
		}
	}

func _generate_coordinate_puzzle(zone_id: String, seed: int) -> Dictionary:
	# Eye shape coordinates (fixed pattern but scaled)
	var base_coords = [
		Vector2(0, 0), Vector2(1, 1), Vector2(2, 2), Vector2(3, 1),
		Vector2(4, 0), Vector2(3, -1), Vector2(2, -2), Vector2(1, -1)
	]
	...
	return {
		...
		"sidekick_view": {
			"sees_roman_numerals": true,
			"coordinates_raw": coords
		}
	}

func _generate_volume_puzzle(zone_id: String, seed: int) -> Dictionary:
	var rng = RandomNumberGenerator.new()
	rng.seed = seed
	...
	return {
		...
		"host_view": {
			"sees_spirit_water_lines": true,
			"target_volume": containers[correct_index].volume
		},
		"sidekick_view": {
			"sees_dimensions": true,
			"containers_data": containers
		}
	}

func _generate_arithmetic_puzzle(zone_id: String, seed: int) -> Dictionary:
	var rng = RandomNumberGenerator.new()
	rng.seed = seed
...
func validate_answer(zone_id: String, player_answer, puzzle_data: Dictionary) -> bool:
	...
	match puzzle_data.type:
		"algebra":
			if player_answer is Dictionary:
				return (player_answer.get("x") == solution.x and 
