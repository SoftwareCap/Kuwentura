...
func get_puzzle_for_zone(zone_id: String) -> Dictionary:
	...
	return {
		...
		"narrative": zone_data.narrative,
		"variation_id": selected.id,
		"seed": seed,
		
		# Views for each role
		"host_view": _build_host_view(zone_data, selected),
		"sidekick_view": _build_sidekick_view(zone_data, selected),
		"ledger": zone_data.ledger,
		
		# Puzzle data and solution
		"data": selected,
		"solution": selected.solution,
		
		# Reward and consequence info
		"reward": zone_data.reward,
		"consequence": zone_data.consequence
	}
